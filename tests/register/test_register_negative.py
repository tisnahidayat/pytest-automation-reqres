...
def test_register_with_empty_data(api):
    data = {
        "email": "",
        "password": ""
    }
    response = api.post("api/register", data=data)
...
@pytest.mark.negative
def test_register_with_empty_email(api):
    data = {
        "email": "",
        "password": "cityslicka"
    }
...
@pytest.mark.negative
def test_register_with_empty_password(api):
    data = {
...
@pytest.mark.negative
def test_register_with_invalid_email(api):
    data = {
        "email": "invalid_email",
        "password": "cityslicka"
    }
    ...
    assert "error" in jsonData, "Response JSON does not contain 'error' field"
    assert isinstance(jsonData["error"], str), "'error' should be a string"
    assert jsonData["error"] == "Note: Only defined users succeed registration", f"Error message mismatch: expected 'Missing email or password', got {jsonData['error']}"

    assert response.elapsed.total_seconds() < 2, "Response took too long"

@pytest.mark.negative
def test_register_without_apikey(api):
    response = api.post("api/register", use_auth=False)
